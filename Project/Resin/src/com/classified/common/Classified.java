 ...
 * @version 0.2, 09 Feb 2003 enhancement
 */
...
/** heading is appended to whatAdSays01 field */      
adTitle = request.getParameter("AdTitle");

      whatAdSays01 = request.getParameter("WhatAdSays01");
      
/** Ad the heading to the WhatAdSays01 field */        
        // Append Heading onto What the ad says.
			//whatAdSays01 = "<center><b>" + adTitle + "</b></center>" 
...
buf.append("Ad Title: ").append(adTitle).append(CR);
...
/// Tom the coommented code below ALL this code is the original !
  ...
  public String toWebString(){
    ...
    buf.append("<li><B>Name:</B> ").append(name).append("<br>");
    ...
    buf.append("<li><B>E-mail:</B> ").append(email).append("<br>");
    buf.append("<li><B>Phone:</B> ").append(phone).append("<br>");
    ///buf.append("<B>Payment Method:</B> ").append(payMethod).append(CR);
    ///buf.append("<B>Credit Card Number:</B> ").append(cardNumber).append("  <B>Expiration Mo:</B> ").append(cardMo).append(" <B>Yr:</B> ").append(cardYr).append(CR);
    buf.append("<li><B>Ad Start Date:</B> ").append(startDate).append("  <B>Ad Ending Date:</B> ").append(endDate).append("<br>");
    buf.append("<li><B>Classified Ad:</B> ").append(classifiedAd).append("<br>");
    ///buf.append("<li><B>Color:</B> ").append(color).append("COLOR</FONT>").append(CR);
    buf.append("<li><B>").append(color).append("</B></FONT>").append("<br>");
    buf.append("<li><B>Banner</B> ").append(banner).append("<br>");     
    
///    buf.append("<li><B>What Ad Says:</B> ").append(CR);
///    buf.append("<li></li>").append(whatAdSays01).append(CR);

//////////// TEST
    buf.append("<br><br><li><B>Ad Title:</B> ").append(adTitle).append("<br>");

    buf.append("<li><B>What Ad Says:</B> ").append(whatAdSays01).append("<br>");
    //buf.append("<li></li>").append(whatAdSays01).append(CR);


//////////// END TEST

    buf.append("<li><B>WebSite:</B> ").append(webSite).append("<br>");
    buf.append("<li><B>Ad I.D. Number: </B>").append(uniqueDateId).append("<br>");
...
/**
  ...
  public String toWebString(){
    ...
    buf.append("<li><B>Banner:</B> ").append(banner).append("<li><li>").append(CR);     
///    buf.append("<li><B>What Ad Says:</B> ").append(CR);
///    buf.append("<li></li>").append(whatAdSays01).append(CR);

//////////// TEST
    buf.append("<li><B>Ad Title:</B></li> ").append(adTitle).append(CR);

    buf.append("<li><B>What Ad Says:</B></li> ").append(whatAdSays01).append(CR);
    //buf.append("<li></li>").append(whatAdSays01).append(CR);
...
*/
  ...
  public String toListString(){
    ...
    buf.append("<BR><B>Banner</B> ").append(banner).append("<br>");
///    buf.append("<BR><B>What Ad Says:</B> ");
///    buf.append("<BR>").append(whatAdSays01);
//////////// TEST
    //buf.append("<BR><B>Ad Title:</B> ").append(adTitle);
    buf.append("<br><br><B>What Ad Says:</B> ").append(whatAdSays01);
...
// changed COLSPAN to 5 for bookmark, change instructions above to 6    
...
buf.append("<tr><td COLSPAN=5 WIDTH=100% VALIGN=TOP BGCOLOR=white>");    
    ...
    buf.append("<FONT ").append(color).append(" FONT size=\"2\" FACE=\"Arial, Helvetica, sans-serif\">");
...
buf.append("<TD NOWRAP BGCOLOR=C9E8F8><FONT SIZE=2 FACE=\"Arial, Helvetica, sans-serif\"><B>").append(rowNumber);    
    buf.append(")</B></FONT>").append(" ");
    buf.append("<FONT COLOR=\"black\" FACE=\"Wingdings\">G</FONT></TD>"); // HAND POINTING UP.
    buf.append("<TD NOWRAP BGCOLOR=C9E8F8>");  

    /**
     * EMAIL link
     * The javascript code below calls the NewWindow() function defined in the
     * SearchDBServlet. The NewWindow function, opens a new window for the user
     * to email the person who listed the ad. The UID is the UniqueDateID key.
     */
                                          // width, height
    buf.append("<A HREF=\"javascript:NewWindow(625,525,'/classified/userEmail?UID="); // MAILBOX    
    buf.append(UID).append("');").append("\"><FONT SIZE=\"2\" COLOR=\"black\" FACE=\"Wingdings\">.</FONT>").append(" ");
    ...
    buf.append(webSite).append("</a>").append("</FONT>");

////////   START TEST BOOK MARK
    /**
     *  BOOK MARK
     * The javascript below calls the BookMarkDBServlet which insert the email address
     * (from the login session) and the UniqueDateId a.k.a. UID into the SAVED_ADS
     * database table.
     */
    buf.append("<TD NOWRAP BGCOLOR=C9E8F8>");
    buf.append("<A HREF=\"javascript:NewWindow(300,300,'/classified/bookmark?UID="); // BOOKMARK
                                                                      // green
    buf.append(UID).append("');").append("\"><FONT SIZE=\"2\" COLOR=\"#008000\" FACE=\"Wingdings\">&</FONT>").append(" ");
    buf.append("<FONT SIZE=\"2\" COLOR=\"008000\" FACE=\"Times New Roman\">bookmark</FONT></A></TD>");    
